...
require_relative "md4_fallback"
#require "openssl" # for BN usage in signer ops
...
class Signer
    ...
    def self.read(io, wmid, password = nil)
      if !password.nil?
        ...
        begin
          ...
          # second attempt with half-password (legacy behaviour in original gem)
      ...
      digest = MD4Fallback::MD4Digest.new
  ...
  def sign(data)
    ...
    digest = MD4Fallback::MD4Digest.new
    digest.update data
    data_words = digest.digest.unpack("V*")

    10.times do
      data_words << SecureRandom.random_number(1 << 32)
    end

    packed = data_words.pack("V*")
    packed = ([packed.length].pack("v") + packed).ljust(@key[:n].num_bytes, 0.chr).reverse
    data_bignum = OpenSSL::BN.new packed, 2
